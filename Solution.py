...
def createTables():
    ...
    try:
        ...
        conn.execute("BEGIN;"
                     ...
                     "CREATE TABLE FilesInDisk(disk_id INTEGER NULL CHECK(disk_id > 0), file_id INTEGER NOT NULL CHECK(file_id > 0), PRIMARY KEY (disk_id, file_id), FOREIGN KEY(disk_id) REFERENCES Disk(disk_id) ON DELETE CASCADE, FOREIGN KEY(file_id) REFERENCES File(file_id) ON DELETE CASCADE);"
                     "CREATE TABLE RAMInDisk(disk_id INTEGER NULL CHECK(disk_id > 0), ram_id INTEGER OT NULL CHECK(ram_id > 0), PRIMARY KEY (disk_id, ram_id), FOREIGN KEY(disk_id) REFERENCES Disk(disk_id) ON DELETE CASCADE, FOREIGN KEY(ram_id) REFERENCES RAM(ram_id) ON DELETE CASCADE );"
                     "COMMIT")
        conn.commit()
...
def dropTables():
    ...
    try:
        ...
        conn.execute("BEGIN;"
                     ...
                     "DROP TABLE Disk;"
                     "DROP TABLE RAM;"
                     "DROP TABLE FilesInDisk;"
                     "DROP TABLE RAMInDisk;"
                     "COMMIT")
        conn.commit()
    except DatabaseException.ConnectionInvalid as e:
        # do stuff
...
def addDisk(disk: Disk) -> Status:
    ...
    try:
        conn = Connector.DBConnector()
        query = sql.SQL("INSERT INTO Disk(disk_id, disk_manufacturing_company, disk_speed ,disk_free_space, disk_cost_per_byte)"
                        "VALUES({id}, {manufacturing_company}, {speed}, {free_space}, {cost_per_byte})").format(id=sql.Literal(disk.getDiskID()),
...
def addDiskAndFile(disk: Disk, file: File) -> Status:
    ...
    try:
        ...
        query = sql.SQL("BEGIN;"
                        ...
                        "VALUES({id_disk}, {manufacturing_company_disk}, {speed_disk}, {free_space_disk}, {cost_per_byte_disk});"
...
#check return values here and also the corectness of the sql query
def addRAMToDisk(ramID: int, diskID: int) -> Status:
    conn = None
    rows_effected = 0
    try:
        conn = Connector.DBConnector()
        query = sql.SQL("INSERT INTO RAMInDisk(disk_id, ram_id) (SELECT Disk.disk_id RAM.ram_id FROM Disk, RAM WHERE Disk.disk_id={disk_id}, RAM.ram_id={ram_id});"
...
def averageFileSizeOnDisk(diskID: int) -> float:
    ...
    try:
        conn = Connector.DBConnector()
        query = sql.SQL("SELECT AVG(file_size) FROM File WHERE File.file_id IN" 
                        "(SELECT file_id from FilesInDisk WHERE disk_id={disk_id} GROUP BY disk_id,file_id").format(disk_id=sql.Literal(diskID))
        rows_effected, _ = conn.execute(query)
        conn.commit()
        # rows_effected is the number of rows received by the SELECT
    except DatabaseException.ConnectionInvalid as e:
...
def diskTotalRAM(diskID: int) -> int:
    ...
    try:
        conn = Connector.DBConnector()
        query = sql.SQL("SELECT SUM(ram_size) FROM RAM WHERE RAM.ram_id IN" 
                        "(SELECT ram_id from RAMInDisk WHERE disk_id={disk_id} GROUP BY disk_id,ram_id").format(disk_id=sql.Literal(diskID))
        rows_effected, _ = conn.execute(query)
        conn.commit()
        # rows_effected is the number of rows received by the SELECT
    except DatabaseException.ConnectionInvalid as e:
